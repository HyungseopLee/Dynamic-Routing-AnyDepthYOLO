...
class SkippableSequentialStages(nn.Sequential):
    ...
    def __init__(self, num_stages, num_patch_merginig, return_idx, *args):
        super().__init__(*args)
    ...
    def forward(self, input, skip: List[bool] = None):
        ...
        for i in range(len(self)):
            ...
            if (i // 2 in self.return_idx and i % 2 == 0):
                output.append(torch.permute(input, [0, 3, 1, 2]))
        return output
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        return_idx=[0, 1, 2, 3],  # @wchkang: return all stages
        freeze_at=-1, 
        freeze_norm=True,
        pretrained=False, 
        ...
        self.return_idx = return_idx # @wchkang: for feature extraction
        ...
        self.features = SkippableSequentialStages(
            len(depths), len(depths) - 1, self.return_idx, *layers)  
        ...
        if freeze_at >= 0:
            self._freeze_parameters(self.pre)
            for i in range(min(freeze_at, len(depths))):
                print(f'[woochul] Freeze parameters in stage {i}')
                self._freeze_parameters(self.features[i])

        if freeze_norm:
            self._freeze_norm(self)

        if pretrained:
            weights_path = "../checkpoint/weights_swin-t-1131-epoch295-super-81.628-base-78.032.pth"
            state = torch.load(weights_path)
            self.load_state_dict(state, strict=False)
            print(f'[woochul] Do Load state_dict from {weights_path}')
    ...
    def forward(self, x, skip=[False, False, False, False]):
        x = self.pre(x)  
        features = self.features(x, skip=skip)

        # for classification. Not needed for feature extraction.
        # x = self.norm(x)
        # x = self.permute(x)
        # x = self.avgpool(x)
        # x = self.flatten(x)
