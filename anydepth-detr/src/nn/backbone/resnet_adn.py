...
donwload_url = {
    50: 'https://github.com/wchkang/depth/releases/download/v1.0.0/weights-resnet50vd_kd_ce_from_resnext101_32x8d_swsl_imagenet21k_1k_T2_T2_step3_finetune.pth',
    101: 'https://github.com/wchkang/depth/releases/download/v1.0.0/weights-resnet101vd_kd_ce_from_resnext101_32x8d_swsl_imagenet21k_1k_T2_T2_step3_finetune.pth',
}
...
class ResNet_ADN(nn.Module):
    def __init__(
        ...
        assert depth in ResNet_cfg, f"expected depth in {ResNet_cfg.keys()}, got {depth}"
        ...
        if pretrained:
            ...
            print(f'Load pretrained PResNet{depth} state_dict')

            '''
            # resnet50vd 82.62/80.24
            weights_path = '../checkpoint/weights-exp014-resnet50vd-kd-ce-T2-T2-step3-finetune.pth'

            # resnet101vd 83.64/81.93
            # weights_path = '../checkpoint/weights-exp016-step3-resnet101vd-kd-cd-T2-T2-bs672-lr0.003-wd3e-5-57epoch.pth'
            
            state = torch.load(weights_path)
            self.load_state_dict(state, strict=False)
            print(f'[woochul] Do Load ResNet{depth}-ADN state_dict from {weights_path}')
            '''
    ...
    def _forward_impl(self, x: Tensor, skip: List[bool] = None) -> List[Tensor]:
    ...
    def forward(self, x: Tensor, skip: List[bool] = None) -> List:
        if skip is None:
