...
\begin{minipage}[t]{0.32\textwidth}
\centering
\includegraphics[width=\linewidth]{fig_calibration_waymo.pdf}
\vspace{2pt}
{\small (c) Waymo}
\end{minipage}
\caption{
...
We measure this on BDD100K (RTX~3090) by comparing each single-path engine run in isolation against an \textsc{alternating} schedule that forces an engine switch on every frame---a worst case far more frequent than any realistic routing trace (Table~\ref{tab:trt}).
The alternating schedule costs only $+1.0\%$ over the mean of the two single-path runs, confirming that engine switching is effectively free and that the two-engine design adds no meaningful deployment overhead.
...
\begin{table}[!t]
\caption{
    TensorRT FP16 engine-switching overhead on BDD100K (RTX~3090, pure GPU inference, CUDA-event timed). \textsc{alternating} forces a BASE$\leftrightarrow$SUPER engine switch every frame (worst case); its latency is within $1\%$ of the mean of the two static single-path engines, so routing between two resident engines is effectively free.
}
\label{tab:trt}
\small
...
\begin{tabular}{lrr}
\toprule
Engine schedule & Latency (ms) & FPS\\
\midrule
\textsc{base} (always-base) & 6.09 & 164.3\\
\textsc{super} (always-super) & 9.70 & 103.0\\
\textsc{alternating} (switch every frame) & 7.97 & 125.5\\
\midrule
mean(base,\,super) & 7.89 & ---\\
switch overhead & \multicolumn{2}{l}{$+0.08$\,ms\ \ ($+1.0\%$)}\\
\bottomrule
\end{tabular}
\end{table}
