...
We isolate this cost on an embedded NVIDIA Jetson Orin Nano (8\,GB unified memory, FP16, clocks pinned with \texttt{jetson\_clocks}) by comparing each single-path engine run against an \textsc{alternating} schedule that forces a BASE$\leftrightarrow$SUPER engine switch on \emph{every} frame---a pathological worst case far more frequent than any realistic routing trace, in which the chosen path typically persists for many consecutive frames.
