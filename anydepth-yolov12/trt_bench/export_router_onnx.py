...
Inputs are the pre-pooled (C,grid,grid) tap groups the detector engine already emits:
  ...
  feat=input -> iv only                                               -> logit
    ...
    python trt_bench/export_router_onnx.py \
        --policy fig8_jetson/assets/policy_both_0.pt \
        --base_engine trt_bench/onnx/bdd_pooled/base.fp16.engine \
        --out trt_bench/onnx/bdd_pooled/router.onnx
"""
import argparse
import sys
...
from method02_advantage_regress_tinyConv.feature_tap import INPUT_LEVEL_LAYERS, PRED_LEVEL_LAYERS
