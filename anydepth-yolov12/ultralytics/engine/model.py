...
class Model(nn.Module, PyTorchModelHubMixin, repo_url="https://github.com/ultralytics/ultralytics", pipeline_tag="object-detection", license="agpl-3.0"):
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        # Extract kwargs that should be passed to the predictor (like skip)
        # Always pass certain parameters to the predictor regardless of args
        predictor_only_params = ['skip']  # Parameters that should always be passed to predictor
        predictor_kwargs = {k: v for k, v in kwargs.items() if k not in args or k in predictor_only_params}
        return self.predictor.predict_cli(source=source) if is_cli else self.predictor(source=source, stream=stream, **predictor_kwargs)
