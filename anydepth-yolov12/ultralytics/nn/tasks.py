...
from ultralytics.utils.loss import (
    ...
    v8PoseLoss,
    v8SegmentationLoss, 
    DetectionLossAnyDepth,  # woochul: YOLOv12 AnyDepth
)
...
class DetectionModelAnyDepth(DetectionModel):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None, skip=None, return_features=False):
        y, dt, embeddings = [], [], []  # outputs
        y_features = []  # features of some layers
        i = 0  # skip index
        for m in self.model:
            # print(f"Layer {m.i}: {type(m)}")
            if m.f != -1:  # if not from previous layer
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
            if profile:
                self._profile_one_layer(m, x, dt)
            ...
            if isinstance(m, (SkippableC3k2, SwitchableC3k2, SwitchableC2f, SwitchableA2C2f, SkippableA2C2f)):
                if isinstance(skip, list) or isinstance(skip, tuple):
                    # print(f"{type(m)}: skip={skip[i]}")  
                    x = m(x, skip[i])
                    i += 1
                else:
                    raise ValueError(
                        f"Expected skip to be a list or tuple, but got {type(skip)}. "
                    )
            elif isinstance(m, (SwitchableConv)):
                # print(f"{type(m)}: skip={skip[i-1]}") 
                x = m(x, skip=skip[max(i, 0)])  # use 'skip' of the next SkippableC3k2/C2f layer
            else:
                # print(f"{type(m)}")  # debug print
                x = m(x)  # run
            ...
            if return_features and m.i in self.save:
                ...
                height_ = 1
                width_ = 1
                # exp #2: make pooled feature lengths between 1k and 2k
                # grid_size = 40 if x.shape[2] >= 80 else 20
                # height_ = x.shape[2] // grid_size
                # width_ = x.shape[3] // grid_size

                x_pooled = nn.functional.adaptive_avg_pool2d(x, (height_, width_))
                y_features.append(x_pooled.view(x.shape[0], -1))  # flatten to [batch_size, channels * height_ * width_]
            if visualize:
                feature_visualization(x, m.type, m.i, save_dir=visualize)
            if embed and m.i in embed:
                embeddings.append(nn.functional.adaptive_avg_pool2d(x, (1, 1)).squeeze(-1).squeeze(-1))  # flatten
        ...
        if return_features:
            return {"pred": x, "features": y_features} # return features if requested
        else:
            return x
    
    def init_criterion(self):
        # return v10DetectLoss(self)
        return E2EDetectLoss(self) if getattr(self, "end2end", False) else v8DetectionLoss(self), DetectionLossAnyDepth(self)

    
    def loss(self, batch, preds=None, preds_base=None):
        """
        Computes the loss for the given batch and predictions, with optional support for knowledge distillation.
        Args:
            batch (Any): The input batch containing ground truth data.
            preds (Any, optional): The model predictions for which the loss is to be computed. Must be provided.
            preds_base (Any, optional): The base predictions from a teacher model for knowledge distillation. If provided, 
                computes the knowledge distillation loss using both student and teacher predictions.
        Returns:
            Any: The computed loss value. If `preds_base` is provided, returns the knowledge distillation loss; 
                otherwise, returns the standard loss.
        Raises:
            AssertionError: If `preds` is not provided.
            AssertionError: If `preds_base` is provided but required attributes for knowledge distillation are not set.
        """
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in (C3k2, SkippableC3k2, SwitchableC3k2):  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m in (A2C2f,): 
                ...
                if scale in "lx":  # for L/X sizes
                    args.append(True)
                    args.append(1.5)
            if m in (SwitchableA2C2f, SkippableA2C2f): # AnyDepth models require residual connections
                legacy = False
                args.append(True) # 'residual=True' for all anydepth models
                if scale in "lx":  # 'mlp_ratio=1.5' for L/X sizes, default is 2.0
                    args.append(1.5)
        elif m is AIFI:
            args = [ch[f], *args]
...
def guess_model_scale(model_path):
    ...
    match = re.search(r"yolo[v]?\d+([nslmx])", Path(model_path).stem)  # noqa, returns n, s, m, l, or x
    if match:
        return match.group(1)
    
    match = re.search(r"yolo-ad-[v]?\d+([nslmx])", Path(model_path).stem)
    if match:
        print("Found YOLOv12 Any-Depth model scale:", match.group(1))
        return match.group(1)
