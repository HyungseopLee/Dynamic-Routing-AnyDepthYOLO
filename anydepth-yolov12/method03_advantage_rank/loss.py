    ...
    def __call__(self, p_super: torch.Tensor,
        ...
        if self.mode == "regress":
            ...
            if rl == "mse":
                ...
                # pairwise margin ranking: for every pair (i,j) with A_i != A_j, want
                # the predicted order to match with a margin. This directly optimises
                # the ordering that the eval threshold uses (Â>tau), ignoring the
                # absolute magnitude of A (which is noisy and irrelevant to routing).
                d_pred = pred.unsqueeze(0) - pred.unsqueeze(1)      # [B,B] Â_i - Â_j
                d_adv = adv_raw.unsqueeze(0) - adv_raw.unsqueeze(1)  # A_i - A_j
                mask = d_adv.abs() > 1e-6
                hinge = torch.clamp(-torch.sign(d_adv) * d_pred + self.margin, min=0.0)
                l_acc = (hinge * mask).sum() / mask.sum().clamp(min=1)
            else:
