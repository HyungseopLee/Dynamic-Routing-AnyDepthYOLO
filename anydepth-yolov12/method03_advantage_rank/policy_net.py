"""Policy network for the 2-level depth decision (method02: tiny-conv router).

Same routing idea as method01 (path B: deterministic, regress A-hat), but the
group projection keeps SPATIAL structure instead of flattening a GAP vector.

Inputs are small per-layer feature GRIDS (built by build_cache.py via adaptive
pooling to GxG), concatenated over layers along the channel dim:
    input_vec : [B, 768, G, G]  (backbone layers 4,6,8 stacked on channels)
    pred_vec  : [B, 640, G, G]  (neck layers 14,17,20)
    path_id   : which path produced these feats (0=BASE,1=SUPER)

Per group (tiny, negligible FLOPs):
    grid [B,C,G,G] -- BN2d -> 1x1 conv (C->d) -> ReLU
                   -> 3x3 depthwise conv (d->d) -> ReLU -> GAP -> [B,d]
then  concat groups + path embed -> FC -> ReLU -> FC -> logit [B,1].
"""
