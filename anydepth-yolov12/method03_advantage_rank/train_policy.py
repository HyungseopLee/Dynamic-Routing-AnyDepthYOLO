...
Usage:
    ...
    python method03_advantage_rank/train_policy.py \
...
from method03_advantage_rank.loss import PolicyLoss
from method03_advantage_rank.policy_net import PolicyNetwork
from method03_advantage_rank.feature_tap import INPUT_LEVEL_LAYERS
...
def feature_aug(inp, noise, cdrop):
    """Feature-space augmentation on the cached grid [B,C,G,G] (train only).
    noise: additive Gaussian std as a fraction of the per-channel std (regulariser
    for the fixed-feature cache, label A preserved). cdrop: channel-dropout prob
    (zero whole channels, inverted-scale like nn.Dropout)."""
    if noise > 0:
        sd = inp.std(dim=(0, 2, 3), keepdim=True)
        inp = inp + torch.randn_like(inp) * (noise * sd)
    if cdrop > 0:
        keep = (torch.rand(inp.shape[0], inp.shape[1], 1, 1, device=inp.device) > cdrop).float()
        inp = inp * keep / (1.0 - cdrop)
    return inp


def run_epoch(net, cache, loader, lossfn, opt=None, prev_p=0.5, aug=None):
    ...
    for (idx,) in loader:
        ...
        if train and aug and (aug["noise"] > 0 or aug["cdrop"] > 0):
            inp = feature_aug(inp, aug["noise"], aug["cdrop"])
...
def main():
    ...
    ap.add_argument("--weight_decay", type=float, default=0.0, help="weight decay (L2 / decoupled for adamw)")
    # --- method03 training-optimisation knobs ---
    ap.add_argument("--optimizer", default="adam", choices=["adam", "adamw"],
                    help="adam (L2 wd) or adamw (decoupled wd)")
    ap.add_argument("--lr_sched", default="const", choices=["const", "cosine"],
                    help="lr schedule over epochs (cosine decays lr->0)")
    ap.add_argument("--warmup", type=int, default=0, help="linear lr warmup epochs (cosine only)")
    ap.add_argument("--aug_noise", type=float, default=0.0,
                    help="feature-space Gaussian noise std (fraction of per-channel std)")
    ap.add_argument("--aug_cdrop", type=float, default=0.0,
                    help="feature-space channel-dropout probability")
    ...
    ap.add_argument("--regress_loss", default="mse", choices=["mse", "mae", "huber", "corr", "rank"],
                    help="regression loss form (regress mode): mse/mae/huber (magnitude), "
                         "corr (Pearson ranking), or rank (pairwise margin ranking; use --margin)")
    ...
    opt = OptCls(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    if args.lr_sched == "cosine":
        ...
        sched = None
    aug = {"noise": args.aug_noise, "cdrop": args.aug_cdrop}
    ...
    for ep in range(args.epochs):
        tr = run_epoch(net, train_cache, train_loader, lossfn, opt, prev_p=args.prev_p, aug=aug)
