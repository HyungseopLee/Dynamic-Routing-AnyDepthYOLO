#!/bin/bash
# Robust, IDEMPOTENT Waymo video eval. Run this in a tmux/screen you own so it
# survives disconnects (unlike harness-launched background jobs, which get reaped).
#
#   tmux new -s waymo
#   bash method02_advantage_regress_tinyConv/run_waymo_eval_robust.sh
#
# 4-way shard (num_shards=4, ids 0..3) covering all 202 segments, run as 2 waves
# across the 2 GPUs. Each shard writes its raw ONLY on completion, so a crash
# loses at most one ~50-segment shard; just re-run and finished shards are skipped.
# Finally merges the 4 raws -> video_curve.json (baselines + 5-seed policy + PI).
set -uo pipefail
...
shard () {  # gpu, shard_id(0..3)
  local GPU=$1 SID=$2 RAW="$O/eval/shard_n${2}.pt"
  ...
  CUDA_VISIBLE_DEVICES=$GPU python -u method02_advantage_regress_tinyConv/eval_video_waymo.py \
    ...
    --raw_out "$RAW" > "$O/eval/video_shard_n${SID}.log" 2>&1
}

# wave 1: shards 0,1 on GPU0,GPU1 ; wave 2: shards 2,3
shard 0 0 & A=$!
shard 1 1 & B=$!
wait $A; wait $B
shard 0 2 & C=$!
...
RAWS=("$O"/eval/shard_n0.pt "$O"/eval/shard_n1.pt "$O"/eval/shard_n2.pt "$O"/eval/shard_n3.pt)
miss=0; for r in "${RAWS[@]}"; do [ -f "$r" ] || { echo "MISSING $r"; miss=1; }; done
if [ $miss -eq 0 ]; then
  python method02_advantage_regress_tinyConv/merge_video_shards.py \
    --shards "${RAWS[@]}" --out "$O/eval/video_curve.json" 2>&1 | tee "$O/eval/merge.log"
