#!/bin/bash
# Finish the feat=BOTH eval: shards 0 and 3 already produced raws; shards 1 and 2
# were reaped mid-run. Re-run just those two in PARALLEL on the two free GPUs,
# then merge all four -> eval_both/video_curve.json. Idempotent: a shard with an
# existing raw is skipped.
set -uo pipefail
...
run_shard () {  # shard_id, gpu
  local SID=$1 GPU=$2 RAW="$O/eval_both/shard_n${1}.pt"
  if [ -f "$RAW" ]; then echo "[$(date +%T)] shard $SID done, skip"; return 0; fi
...
if [ $miss -eq 0 ]; then
  ...
  echo "[$(date +%T)] shards still missing; re-run."
fi
