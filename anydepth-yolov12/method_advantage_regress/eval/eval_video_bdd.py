...
Usage:
    python -m method_advantage_regress.eval.eval_video_bdd \
        --weight finetuning_AnyDepthYOLO/weights/bdd100k/30e_..._alpha0.2_orig_mAP35.1_33.8.pt \
...
from method_advantage_regress.router.feature_tap import INPUT_LEVEL_LAYERS, PRED_LEVEL_LAYERS, STATE_LAYERS
...
def main():
    ...
    if args.val_cache:
        ...
        v_in = vc["input_base"].float().to(device); v_pr = vc["pred_base"].float().to(device)
