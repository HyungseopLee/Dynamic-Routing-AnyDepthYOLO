...
def main():
    ...
    def online_loop(order, Ltgt, fam):
        ...
        # The label-count `n` can exceed the number of frames the .mov actually decodes
        # (the last few labelled frames may have no decodable image); trim to what we ran
        # so trailing un-measured slots don't pollute the smoothed latency with zeros.
