...
Usage:
    python -m method_advantage_regress.analysis.plot_path_consistency \
        --cache method_advantage_regress/outputs/bdd100k/cache_val_both.pt \
        --out   method_advantage_regress/outputs/figures/fig_path_consistency_bdd.pdf
"""
import argparse
import sys
...
def main():
    ...
    ap.add_argument("--cache",  default="method_advantage_regress/outputs/bdd100k/cache_val_both.pt")
    ap.add_argument("--out",    default="method_advantage_regress/outputs/figures/fig_path_consistency_bdd.pdf")
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--n_pts",  type=int, default=4000,
                    help="points to scatter per panel (subsampled for clarity)")
    args = ap.parse_args()

    BDD = "method_advantage_regress/outputs/bdd100k"
    GROUPS = [
