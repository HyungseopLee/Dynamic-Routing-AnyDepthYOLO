...
router family names: both_p00_s*, both_p05_s*, both_p10_s*
  -> prev_p = 0.0 / 0.5 / 1.0
    ...
    python -m method_advantage_regress.ablation.plot_ablation_prevp \
        --curve method_advantage_regress/outputs/bdd100k/eval/video_curve_prevp_bdd.json
"""
import argparse
import json
import re
...
def main():
    ...
    ap.add_argument("--curve", default="method_advantage_regress/outputs/kitti/eval/video_curve_prevp_both_g2.json")
    ap.add_argument("--metric", default="map", choices=["map", "map50"])
    ap.add_argument("--out", default="method_advantage_regress/outputs/figures/fig_abl_prevp_ap5095.pdf")
    args = ap.parse_args()
