...
#   results/step3_eval/bdd100k/
#
# Usage (from the repo root):
#   sudo jetson_clocks
#   bash step4_deploy/bench/run_ablation_experiments.sh 2>&1 | tee ablation_run.log
...
for i in "${!EXPERIMENTS[@]}"; do
    ...
    if PYTHONPATH=. python3 \
           step4_deploy/online_budget_demo_stream.py \
           $COMMON $EXP; then
...
log "JSONs in: results/step3_eval/bdd100k/"
ls -lh results/step3_eval/bdd100k/jetson_trtengine_720*b0.85*.json 2>/dev/null || true
