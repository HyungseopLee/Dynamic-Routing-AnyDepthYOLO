...
def main():
    ...
    for fam in fams:
        ...
        # display values updated once per second (every out_fps frames)
        disp_fps_realized = 1000.0 / L_ema
        disp_fps_target   = float(fps_tgt_arr[0])
        fps_update_interval = max(1, int(round(args.out_fps)))
        ...
        for bgr, first, seg_label in stream_family(order):
            ...
            if frame_idx % fps_update_interval == 0:
                disp_fps_realized = fps_realized
                disp_fps_target   = fps_tgt_now

            fam_title = FAMILY_TITLES.get(fam, fam)
            scene_str = f"{fam_title}  [{seg_label}]"
            out_frame = draw_overlay(disp, boxes, config, disp_fps_realized, disp_fps_target,
