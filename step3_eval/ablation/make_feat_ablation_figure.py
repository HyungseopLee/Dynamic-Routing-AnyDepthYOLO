...
seeds, with a twin GFLOPs/frame top axis.
    ...
    python -m step3_eval.ablation.make_feat_ablation_figure \
        --curve results/step3_eval/bdd100k/eval/video_curve_archabl.json \
        --out results/figures/fig_feat_ablation_bdd_ap5095.pdf
    python -m step3_eval.ablation.make_feat_ablation_figure \
        --curve results/step3_eval/kitti/eval/video_curve_featcomp.json \
        --dataset kitti \
        --out results/figures/fig_feat_ablation_kitti_ap5095.pdf
"""
import argparse
import json
import re
...
def main():
    ...
    ap.add_argument("--out", default="results/figures/fig_abl_feat_ap5095.pdf")
    args = ap.parse_args()
