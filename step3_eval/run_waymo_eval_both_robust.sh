...
ROUTERS=results/step2_router/weights/waymo
CACHE=results/step2_router/cache/waymo
O=results/step3_eval/waymo
W=results/step1_finetune/weights/waymo/best.pt
POL=$(for s in 0 1 2 3 4; do echo -n "seed$s=$ROUTERS/router_both_$s.pt,"; done | sed 's/,$//')
mkdir -p "$O/eval_both"
