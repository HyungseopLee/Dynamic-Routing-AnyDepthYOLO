...
class SkippableRepVggBlock(nn.Module):
...
class SkippableCSPRepLayer(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 num_blocks=3,
                 ...
                 bias=None,
                 act="silu"):
        super(SkippableCSPRepLayer, self).__init__()
        ...
        for i in range(num_blocks):
            blocks.append(SkippableRepVggBlock(hidden_channels, hidden_channels, act=act, skippable=(i >= num_shared_blocks)))
...
# transformer
class SkippableTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model,
                 nhead,
...
class SkippableTransformerEncoder(nn.Module):
...
@register
class HybridEncoderADN(nn.Module):
    def __init__(self,
        ...
        # encoder transformer
        encoder_layer = SkippableTransformerEncoderLayer(
            hidden_dim, 
            nhead=nhead,
        ...
        self.encoder = nn.ModuleList([
            SkippableTransformerEncoder(copy.deepcopy(encoder_layer), num_encoder_layers) for _ in range(len(use_encoder_idx))
        ])
        ...
        for _ in range(len(in_channels) - 1, 0, -1):
            ...
            self.fpn_blocks.append(
                ...
                # orig
                SkippableCSPRepLayer(hidden_dim * 2, hidden_dim, round(3 * depth_mult), act=act, expansion=expansion)
            )
...
class HybridEncoder(nn.Module):
    ...
    def forward(self, feats, skip=None):
        assert len(feats) == len(self.in_channels)
