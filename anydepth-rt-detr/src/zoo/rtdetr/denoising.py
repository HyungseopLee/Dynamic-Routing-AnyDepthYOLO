...
def get_contrastive_denoising_training_group(targets,
    ...
    # print(f"input_query_class: {input_query_class.shape}")
    # print(f"input_query_bbox: {input_query_bbox.shape}")
    # print(f"pad_gt_mask: {pad_gt_mask.shape}")
    ...
    # print(f"input_query_class2: {input_query_class.shape}")
    # print(f"input_query_bbox2: {input_query_bbox.shape}")
    # print(f"pad_gt_mask2: {pad_gt_mask.shape}")
    
    # positive and negative mask
    negative_gt_mask = torch.zeros([bs, max_gt_num * 2, 1], device=device)
    negative_gt_mask[:, max_gt_num:] = 1
    negative_gt_mask = negative_gt_mask.tile([1, num_group, 1])
    # print(f"negative_gt_mask: {negative_gt_mask.shape}")
    positive_gt_mask = 1 - negative_gt_mask
    # print(f"positive_gt_mask: {positive_gt_mask.shape}")
    # contrastive denoising training positive index
    positive_gt_mask = positive_gt_mask.squeeze(-1) * pad_gt_mask
    # print(f"positive_gt_mask2: {positive_gt_mask.shape}")
    dn_positive_idx = torch.nonzero(positive_gt_mask)[:, 1]
    # print(f"dn_positive_idx: {dn_positive_idx.shape}")
    dn_positive_idx = torch.split(dn_positive_idx, [n * num_group for n in num_gts])
    # print(f"len(dn_positive_idx2): {len(dn_positive_idx)}")
    # print(f"dn_positive_idx2: {dn_positive_idx}")
